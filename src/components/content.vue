...
.content
	...
	.content__body
		...
		p.
			1 По таблице 6 для {{translate(getVertical)}}, скорости ветра {{getState.wind}} м/с и расстояния {{getState.distance < 10 ? "менее" : "более"}} 10 км находим скорость переноса облака зараженного воздуха, которая равна {{getState.avgSpeed}} м/с.
		p.
		...
		p.
			2 По таблице 7 находим, что для {{getState.size}} т {{translate(getState.matter)}} при температуре {{getState.temp}}˚С продолжительность поражающего действия составляет {{getState.time2}} ч.

		p
			| 3 Строим график. По оси абсцисс откладываем {{getState.G}} км, по оси ординат – время, равное {{getState.time2}} ч. Соединяем точки А и Б прямой. Восстанавливаем перпендикуляр с точки, равной {{getState.distance}} км до пересечения с прямой АБ (точка В). Из точки В проводим прямую до оси ординат. Находим, что поражающее действие {{translate(getState.matter)}} на расстоянии {{getState.distance}} км от участка аварии составляет {{getState.coords.smallY.toFixed(2)}} ч.
